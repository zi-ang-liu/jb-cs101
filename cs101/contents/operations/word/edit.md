...
<img src="images/word_table_design.png" alt="表のデザイン" width="600px">
...
<img src="images/word_table_input.png" alt="表の入力" width="600px">
...
図番号は図の下に、表番号は表の上に挿入されます。図表番号を挿入した後に、図や表を追加した場合、図表番号は自動的に更新されます。

:::{note}
図表番号の後ろに、一般的には図表の内容を説明するための**キャプション**を挿入します。
:::
...
1. 図表を参照したい位置にカーソルを置きます。
2. 「参考資料」タブの「図表」グループから、「相互参照」を選択します。
3. 「相互参照」ダイアログが表示されます。
4. 「参照する項目」から「図」または「表」を選択し、「挿入」をクリックします。
5. 「OK」をクリックします。