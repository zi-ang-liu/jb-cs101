...
**フォルダー**(folder) とは、ファイルを整理して保存するための場所で、他のフォルダーを含むこともできます。
...
### フォルダーを作成してみよう
1. デスクトップで右クリックし，「新規作成」→「フォルダー」を選択します。
2. フォルダー名に「foo」と入力し、<kbd>Enter</kbd> キーを押します。
...
:::note
テキストファイルを新規作成する場合、自動的に「.txt」が付加されます．そのため，ファイル名に「.txt」を手動で入力する必要はありません。
:::
...
新規作成したテキストファイルは自動的に「.txt」という文字列が付きます。これは**拡張子**（file name extension）と呼ばれ、ファイルの種類を示すために使われます。
