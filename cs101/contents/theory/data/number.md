...
コンピューターはどのように働くかを理解するためには、2進数の理解が不可欠である。また、8進数や16進数も、2進数との関係が深く、コンピューターではよく使用される。これらの記数法を理解することで、コンピューターの動作やデータの表現方法をより深く理解できるようになる。
...
私たちが普段使う10進法は、0から9の数字を用いて数を表現する。各数字の位置によってその値が決まる。例えば、123という数は、以下のように表現される：
...
:::{note}
古代中国では、算木を使って計算を行っていた。「孫子算経」では、算木の位置によって数を表現する方法が説明されている。「凡算之法、先識其位」は、「凡そ算の法は、其の位を識るを先とす」と訳される。
